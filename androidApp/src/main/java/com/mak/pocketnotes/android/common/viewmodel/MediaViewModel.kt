...
class LegacyMediaViewModel(
  ...
  fun onUIEvents(event: UIEvent) {
    viewModelScope.launch {
      when (event) {
        UIEvent.Backward -> {
          serviceHandler.onPlayerEvents(MediaEvent.Backward)
        }

        UIEvent.Forward -> {
          serviceHandler.onPlayerEvents(MediaEvent.Forward)
        }
        ...
        is UIEvent.SeekTo -> {
          serviceHandler.onPlayerEvents(
            MediaEvent.SeekTo,
            seekPosition = ((duration * event.position) / 100f).toLong()
          )
        }

        UIEvent.SeekToNext -> {
          serviceHandler.onPlayerEvents(MediaEvent.SeekToNext)
        }
