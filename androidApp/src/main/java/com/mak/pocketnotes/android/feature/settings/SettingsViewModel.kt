  ...
  fun onAction(action: SettingsAction) {
    when (action) {
      is SettingsAction.OnThemeChange -> {
        update { it.copy(theme = action.theme) }
      }

      is SettingsAction.OnLanguageChange -> {
        update { it.copy(language = action.lang) }
      }

      is SettingsAction.OnTextSizeChange -> {
        update { it.copy(textSize = action.size) }
      }

      is SettingsAction.OnAutoPlayNextChange -> {
        update { it.copy(autoPlayNext = action.enabled) }
      }

      is SettingsAction.OnDownloadWifiOnlyChange -> {
        update { it.copy(downloadOnWifiOnly = action.enabled) }
      }

      is SettingsAction.OnDeleteAfterPlayedChange -> {
        update { it.copy(episodeDeleteAfterPlayed = action.enabled) }
      }

      is SettingsAction.OnStreamQualityChange -> {
        update { it.copy(streamingQuality = action.quality) }
      }
      ...
      SettingsAction.OnOpenSourceCode -> {
        viewModelScope.launch { _events.send(SettingsEvent.OpenUrl("https://github.com/app")) }
      }

      SettingsAction.OnOpenLicenses -> {
        viewModelScope.launch { _events.send(SettingsEvent.NavigateToLicenses) }
      }
    }
  }
