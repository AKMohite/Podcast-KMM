...
//        }
  ...
  override fun observe(): Flow<List<Genre>> = genresDAO
    .getGenres()
    .map {
      it.asGenres()
  ...
  private suspend fun fetch(): List<GenreDTO> = api
    .getAllGenres()
    .getOrThrow()
    .genres
    .orEmpty()

  private fun List<GenreDTO>.asGenreEntities(): List<GenreEntity> = map { dto ->
    GenreEntity(
      id = dto.id!!,
      title = dto.name ?: "",
  ...
  private fun List<GenreEntity>.asGenres(): List<Genre> = map { entity ->
    Genre(
      id = entity.id,
      name = entity.title,
